...
const (
	organizationVPCPeeringResource = "aiven_aws_org_vpc_peering_connection"
)
...
func TestAccAivenAWSOrgVPCPeeringConnection(t *testing.T) {
	var (
		...
		registry       = preSetVPCPeeringTemplates(t)
		...
		Steps: []resource.TestStep{
			{
				Config: newComposition().
					...
					Add(organizationVPCPeeringResource, map[string]any{
						"resource_name":  "test_org_vpc_peering",
						"vpc_id":         "aiven_organization_vpc.test_org_vpc.id",
...
func TestAccAivenAWSOrgVPCPeeringConnectionFull(t *testing.T) {
	...
	var (
		...
		resourceName = fmt.Sprintf("%s.%s", organizationVPCPeeringResource, "test_peering")
		...
		Steps: []resource.TestStep{
			{
				Config: newComposition().
					...
					Add(organizationVPCPeeringResource, map[string]any{
						...
						"vpc_id":         "aiven_organization_vpc.example.id",
...
func testAccCheckAWSOrgVPCPeeringResourceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != organizationVPCPeeringResource {
			continue
		}
...
func preSetVPCPeeringTemplates(t *testing.T) *acc.TemplateRegistry {
	t.Helper()

	registry := acc.NewTemplateRegistry(organizationVPCPeeringResource)
...
resource "aiven_organization_vpc" "{{ .resource_name }}" {
	...
	registry.MustAddTemplate(t, organizationVPCPeeringResource, `
...
data "aiven_aws_org_vpc_peering_connection" "{{ .resource_name }}" {
    organization_id        	= data.aiven_organization.foo.id
    vpc_id         		   	= {{ .vpc_id }}
    peering_connection_id 	= split("/", aiven_aws_org_vpc_peering_connection.test_org_vpc_peering.id)[2]
}`)
