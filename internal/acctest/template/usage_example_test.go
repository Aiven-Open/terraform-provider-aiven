...
func TestExpectedBehaviour(t *testing.T) {
	templateSet := GetTemplateSet(t) // all existing resources are pre-registered
...
provider "google" {
	...
	builder := templateSet.NewBuilder().
		...
		AddResource("aiven_kafka_quota", map[string]any{
			"resource_name":      "example_quota_1", // add a resource
			"project":            "test_project",
		...
		AddResource("aiven_kafka_quota", map[string]any{ // add a second with different resource name
		...
		Add("gcp_provider", map[string]any{"gcp_project": "test-project", "gcp_region": "us-central1"})

	str := builder.MustRender(t) // render the template
	assert.NotEmpty(t, str)

	builder.RemoveByResourceName("example_quota_1") // remove a resource by its name
